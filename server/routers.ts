...
export const savedAddressInputSchema = z.object({
  label: z.string().trim().min(1).max(80), province: z.string().trim().min(1).max(80), postalCode: z.string().regex(/^\d{5}$/).or(z.literal("")).default(""), district: z.string().trim().min(1).max(100),
