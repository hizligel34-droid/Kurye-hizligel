...
export const savedAddressInputSchema = z.object({
  ...
  if (!isValidTurkishPostalCode(input.postalCode)) refinement.addIssue({ code: z.ZodIssueCode.custom, path: ["postalCode"], message: "Posta kodu 5 haneli olmalı" }); if (!isValidBuildingNo(input.buildingNo)) refinement.addIssue({ code: z.ZodIssueCode.custom, path: ["buildingNo"], message: "Geçerli bina/kapı numarası gerekli" });
});
